...
RUN --mount=type=cache,target=/musl sh -c "source /app/crossfiles/autoenv.sh && cmake ../libde265 -DBUILD_SHARED_LIBS=false -DENABLE_DECODER=false -DCMAKE_TOOLCHAIN_FILE=/app/crossfiles/toolchain.cmake"
RUN --mount=type=cache,target=/musl make -j $(nproc)
RUN --mount=type=cache,target=/musl cmake --install . --prefix /heif
...
RUN --mount=type=cache,target=/musl sh -c "source /app/crossfiles/autoenv.sh && \
...
RUN --mount=type=cache,target=/musl sh -c "source /app/crossfiles/autoenv.sh && cp \${STDC_LIBS}/libstdc++.* /heif/lib/" 
ENV LIBHEIF_LINK_CXX=dynamic
#ENV LIBHEIF_LINK_CXX=static
...
USER proxy
COPY asset ./asset
COPY --from=build_app /app/libs/* /lib
